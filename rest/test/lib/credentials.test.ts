...
describe('extractCredentials', () => {
  describe('Authorization: Basic header', () => {
    it('extracts user and password from a valid Basic auth header', () => {
      const headers = makeHeaders({
        authorization: basicAuth('user@example.com', 's3cr3t'),
      });
    ...
    it('handles passwords containing colons', () => {
      const headers = makeHeaders({
        authorization: basicAuth('user@example.com', 'p:a:s:s'),
      });
      const creds = extractCredentials(headers);
      expect(creds.user).toBe('user@example.com');
    ...
    it('throws on a non-Basic Authorization scheme', () => {
      const headers = makeHeaders({ authorization: 'Bearer some-token' });
      expect(() => extractCredentials(headers)).toThrow('Invalid Authorization header: expected Basic scheme');
    });

    it('throws when the decoded value has no colon', () => {
      const noColon = 'Basic ' + Buffer.from('nocolon').toString('base64');
      const headers = makeHeaders({ authorization: noColon });
      expect(() => extractCredentials(headers)).toThrow('Invalid Authorization header: missing colon separator');
    });

    it('throws when user is empty', () => {
      const headers = makeHeaders({ authorization: basicAuth('', 'password') });
      expect(() => extractCredentials(headers)).toThrow('Invalid Authorization header: user and password are required');
    });

    it('throws when password is empty', () => {
      const headers = makeHeaders({ authorization: basicAuth('user@example.com', '') });
      expect(() => extractCredentials(headers)).toThrow('Invalid Authorization header: user and password are required');
    });
  });
  ...
  describe('X-Mail-* headers (fallback)', () => {
    ...
    it('throws listing missing header names when only one is absent', () => {
      const headers = makeHeaders({ 'x-mail-user': 'user@example.com' });
